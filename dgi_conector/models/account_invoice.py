...
# <!-- Empresa [String(10)] -->
    ...
    txt_compute_estado = fields.Text()
    txt_compute_defff = fields.Text()
...
# <!-- Tipo CFE Se puede grabar el codigo del CFE o el tipo de    doc. del ERP, para esto debe existir la relación[Integer] -->
        cfe_type = "111"
        if self.l10n_co_edi_type == "1":
                cfe_type = "111"
        if self.l10n_co_edi_type == "2":
                cfe_type = "101"
        if self.l10n_co_edi_operation_type == "20" and cfe_type == "111":
                cfe_type = "112"
        if self.l10n_co_edi_operation_type == "20" and cfe_type == "101":
                cfe_type = "102"
        if self.l10n_co_edi_operation_type == "30" and cfe_type == "111":
                cfe_type = "113"
        if self.l10n_co_edi_operation_type == "30" and cfe_type == "101":
                cfe_type = "103"

        dni_name = self.partner_id.l10n_latam_identification_type_id.name
        user_country = self.partner_id.country_id.name
        rec_doc = "4"
        if dni_name == "VAT" or dni_name == "CUIT" or dni_name == "RUT" or dni_name == "CUIT" or dni_name == "Cédula Extranjera":
                rec_doc = "7"
        if user_country == "Uruguay" and (dni_name == "RUC" or dni_name == "RUT" or dni_name == "VAT"):
                rec_doc = "2"
        if user_country == "Uruguay" and dni_name == "C.I":
                rec_doc = "3"
        if (dni_name == "DNI" or dni_name == "Cédula Extranjera") and user_country in ["Argentina","Brasil","Chile","Paraguay"] :
                rec_doc = "6"
        if dni_name == "Pasaporte":
                rec_doc = "5"     
...
# <!-- Fecha del CFE [Date] -->
        ...
        text_node = doc.createTextNode(str(self.invoice_date))
...
# <!-- Dirección Fiscal Emisor [String(70)] -->
        ...
        text_node = doc.createTextNode(str(self.company_id.street_name))
...
# <!-- Direccion del receptor [String(70)] -->
        if self.partner_id.street_name:
                ...
                text_node = doc.createTextNode(str(self.partner_id.street_name))
...
# 10: Exportación y asimiladas 11: Impuesto percibido 12:IVA en suspenso [Integer] VALORES PLIS-->
            ind_fact = "1"
            if line.tax_ids.name == "IVA Ventas (22%)":
                ind_fact = "3"
                monto_iva_base += line.price_subtotal
                price_unit = line.price_unit * 1.22
            if line.tax_ids.name == "IVA Ventas (10%)":
                ind_fact = "2"
