...
class AccountExportSircar(models.Model):
    ...
    export_sircar_data = fields.Text("File content")
            ...
            for line in pay.payment_ids:
                if line.payment_method_id.code == "withholding":
                    for (
                        ...
                        if (
                            retSIRCAR in tax_line.tag_ids.ids
                            and jurSIRCAR in tax_line.tag_ids.ids
                        ):
                            if not pay in ret:
                                ret += pay
            ...
            for line in inv.invoice_line_ids:
                for tax in line.tax_ids:
                    for tax_line in tax.invoice_repartition_line_ids:
                        if (
                            percSIRCAR in tax_line.tag_ids.ids
                            and jurSIRCAR in tax_line.tag_ids.ids
                        ):
                            if not inv in per:
                                per += inv
            ...
            if rec.doc_type == WITHHOLDING:
                ...
                if tag_tax.jurisdiction_code != "904":
                    ...
                    for payment in payments:
                        ...
                        _date = payment.payment_date.strftime(
                            "%d/%m/%Y"
                        )  # fecha del pago
                        line += _date + ","
                        ...
                        for line_alicuot in payment.partner_id.arba_alicuot_ids:
                            if (
                                line_alicuot.tag_id.id == jurSIRCAR
                                and rec.date_from >= line_alicuot.from_date
                                and rec.date_to <= line_alicuot.to_date
                            ):
                ...
                if tag_tax.jurisdiction_code != "904":
                    ...
                    for invoice in invoices:
                        ...
                        amount_total = formatted_amount = "{:.2f}".format(
